...
describe('Oxide.Client.AbstractClient', function () {
  it('obeys given arguments', function () {
    const client = new Oxide.Client.AbstractClient({ host: 'foo', port: 'bar' })
  ...
  it('set values to their defaults when no arguemnts are provided', function () {
    const client = new Oxide.Client.AbstractClient({ host: 'foo' })
  ...
  it('discards values that aren\'t in the default values', function () {
    const client = new Oxide.Client.AbstractClient({ randomProperty: 'randomValue' })

    expect(client).to.not.have.property('randomProperty')
  })

  it('returns a socket with #connect', function () {
    const client = new Oxide.Client.AbstractClient()
  ...
  it('calls #send at an interval', function (done) {
    const client = new Oxide.Client.AbstractClient({ interval: 1 })
  ...
  describe('#send', function () {
    ...
    const pickle = Oxide.Protocol.Pickle()

    const m1 = new Oxide.Metrics.CarbonMetric({ path: 'foo', value: 'bar' })
    const m2 = new Oxide.Metrics.CarbonMetric({ path: 'foo', value: 'bar' })
